...
public class MoodView extends VerticalLayout {
    ...
    private HorizontalLayout createLegend() {
        ...
        legend.add(createLegendItem("Neutral", "#ffeb3b"));
    ...
    private void refreshCalendar() {
        ...
        // Load all mood entries for the year
        LocalDate yearStart = LocalDate.of(currentYear, 1, 1);
        LocalDate yearEnd = LocalDate.of(currentYear, 12, 31);
        List<MoodEntry> entries = service.findByDateRange(yearStart, yearEnd);

        Map<LocalDate, Mood> moodMap = entries.stream()
                .collect(Collectors.toMap(MoodEntry::getEntryDate, MoodEntry::getMood));

        // Create calendar for each month
        for (Month month : Month.values()) {
            calendarContainer.add(createMonthCalendar(month, moodMap));
        }
    }

    private VerticalLayout createMonthCalendar(Month month, Map<LocalDate, Mood> moodMap) {
                ...
                if (date.getMonth() == month) {
                    ...
                    if (mood != null) {
                        ...
                        moodBox.getStyle()
                                ...
                                .set("background-color", getMoodColor(mood));
                        cell.add(moodBox);
    ...
    private String getMoodColor(Mood mood) {
        return switch (mood) {
            case GOOD -> "#4caf50";    // Green
            case NEUTRAL -> "#ffeb3b"; // Yellow
            case BAD -> "#f44336";     // Red
        };
    }
}
