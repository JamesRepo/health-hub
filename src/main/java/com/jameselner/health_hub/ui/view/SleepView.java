...
public class SleepView extends VerticalLayout {
    ...
    private HorizontalLayout createLegend() {
        ...
        legend.add(createLegendItem("Moderate", "#ffeb3b"));
        legend.add(createLegendItem("Poor", "#f44336"));
    ...
    private void refreshCalendar() {
        ...
        // Load all sleep entries for the year
        LocalDate yearStart = LocalDate.of(currentYear, 1, 1);
        LocalDate yearEnd = LocalDate.of(currentYear, 12, 31);
        List<SleepEntry> entries = service.findByDateRange(yearStart, yearEnd);

        Map<LocalDate, SleepQuality> sleepMap = entries.stream()
                .collect(Collectors.toMap(SleepEntry::getEntryDate, SleepEntry::getQuality));

        // Create calendar for each month
        for (Month month : Month.values()) {
            calendarContainer.add(createMonthCalendar(month, sleepMap));
        }
    }

    private VerticalLayout createMonthCalendar(Month month, Map<LocalDate, SleepQuality> sleepMap) {
        ...
        while (weekStart.isBefore(lastOfMonth) || weekStart.equals(lastOfMonth) ||
            ...
            for (int i = 0; i < 7; i++) {
                ...
                if (date.getMonth() == month) {
                    SleepQuality quality = sleepMap.get(date);
                    if (quality != null) {
                        Div qualityBox = new Div();
                        qualityBox.getStyle()
                                .set("width", "20px")
                                .set("height", "20px")
                                .set("border-radius", "3px")
                                .set("background-color", getQualityColor(quality));
                        cell.add(qualityBox);
    ...
    private String getQualityColor(SleepQuality quality) {
        return switch (quality) {
            case GOOD -> "#4caf50";     // Green
            case MODERATE -> "#ffeb3b"; // Yellow
            case POOR -> "#f44336";     // Red
        };
    }
}
